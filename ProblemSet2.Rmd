...
```{r}
p7 <- rdplot(y = bbdd$GraduatedIn2021,
...
ggplot2::ggsave("./outputs/P7_rd_placebo_graduated2021.png", plot = p7$rdplot, width = 8, height = 6, dpi = 300)
```
...
# --- Crear gráfico RD (sin covariables, solo visualización) ---
p8 <- rdplot(
  y = bbdd$GraduatedIn2021,
  x = bbdd$SchoolDaysFromMay302022,
  c = 0,
  p = 1,
  x.label = "Días escolares desde 30 de mayo 2022",
...
# --- Guardar el gráfico en alta resolución ---
ggsave("./outputs/P8_rd_placebo_cov_graduated2021.png", plot = p8$rdplot, width = 8, height = 6, dpi = 300)
```
...
```{r}
# Pregunta 10b: RD Fuzzy con Variables Instrumentales (2SLS)
...
# Ajuste del modelo
iv_model <- ivreg(
  EnrolledByAug312022 ~ ListedInDropoutReport * SchoolDaysFromMay302022 +
    factor(IDschool) + factor(DayOfWeekDroppedOut) |
    DroppedOutMay30Before * SchoolDaysFromMay302022 +
    factor(IDschool) + factor(DayOfWeekDroppedOut),
  data = bbdd
)

# Mostrar solo coeficientes que contengan las variables relevantes
coefs <- coef(summary(iv_model))
coefs[grep("ListedInDropoutReport|SchoolDaysFromMay302022|Intercept", rownames(coefs)), ]


# --- Guardar la primera etapa por separado ---
modelsummary(
  first_stage,
  output = "./outputs/p10_first_stage.html",
  title = "Primera Etapa (First Stage) - RD Fuzzy",
  stars = TRUE,
  gof_omit = "AIC|BIC|Log.Lik|RMSE",
  coef_map = c(
    "(Intercept)" = "Intercepto",
    "DroppedOutMay30Before" = "Desertó antes del 30 de mayo",
    "SchoolDaysFromMay302022" = "Días escolares desde el 30 de mayo",
    "DroppedOutMay30Before:SchoolDaysFromMay302022" = "Interacción: Deserción × Días"
  ),
  fmt = 3
)

# --- Guardar el modelo IV por separado ---
modelsummary(
  iv_model,
  output = "./outputs/p10_iv.html",
  title = "Modelo IV (2SLS) - RD Fuzzy",
  stars = TRUE,
  gof_omit = "AIC|BIC|Log.Lik|RMSE",
  coef_map = c(
    "(Intercept)" = "Intercepto",
    "ListedInDropoutReport" = "Listado en reporte de deserción",
    "SchoolDaysFromMay302022" = "Días escolares desde el 30 de mayo",
    "ListedInDropoutReport:SchoolDaysFromMay302022" = "Interacción: Reporte × Días escolares"
  ),
  fmt = 3
)
